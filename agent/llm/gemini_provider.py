        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if resp.status_code in RETRYABLE_STATUS_CODES and attempt < MAX_RETRIES - 1:
                    wait = 2 ** (attempt + 1)
                    logger.warning(
                        "Gemini API %d, retrying in %ds (%d/%d)...",
                        resp.status_code, wait, attempt + 1, MAX_RETRIES,
                    )
                    await asyncio.sleep(wait)
                    continue
                if resp.status_code != 200:
                    raise LLMAPIError(f"Gemini API returned {resp.status_code}: {resp.text[:200]}")
                ...
                logger.debug("Gemini call took %.1fs", time.monotonic() - start)
                return text
                ...
                if attempt < MAX_RETRIES - 1:
                    wait = 2 ** (attempt + 1)
                    logger.warning("Gemini HTTP error, retrying in %ds: %s", wait, e)
                    await asyncio.sleep(wait)
                    continue
                raise LLMAPIError(f"Gemini API failed: {e}") from e

    raise LLMAPIError(f"Gemini API failed after {MAX_RETRIES} retries: {last_error}") from last_error
